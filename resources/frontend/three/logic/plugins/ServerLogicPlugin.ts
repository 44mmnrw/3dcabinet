...
import { PHYSICAL } from '../../constants/PhysicalConstants.ts';
...
export interface ServerCalculations {
    ...
    coolingAdequacy: 'unknown' | 'none' | 'insufficient' | 'near_limit' | 'adequate' | 'passive_sufficient';
    airflowType: string;
    airflowBalance: 'unknown' | 'restricted' | 'balanced' | 'moderate';
    redundancy: string;
    redundancyStatus: 'unknown' | 'ok' | 'at_risk' | 'acceptable';
    isOverloaded: boolean;
    needsActiveCooling: boolean;
    needsRedundancy: boolean;
    [key: string]: unknown;
}
...
export class ServerLogicPlugin extends LogicPlugin {
    ...
    calculate(cabinetType: CabinetType, equipmentList: EquipmentConfig[]): ServerResult {
        const calculations: ServerCalculations = {
            // Мощность
            totalPower: 0,              // кВт
            totalHeatLoad: 0,           // кВт (тепловыделение)
            usedRackUnits: 0,
            powerDensity: 0,            // кВт/U
            maxPowerDensity: (cabinetType as any).power?.maxPowerDensity || 0.5,
            
            // Охлаждение
            coolingRequired: 0,         // кВт
            coolingAvailable: (cabinetType as any).cooling?.coolingCapacity || 0,
            coolingAdequacy: 'unknown',
            airflowType: (cabinetType as any).cooling?.airflowType || 'passive',
            airflowBalance: 'unknown',
            
            // Резервирование
            redundancy: (cabinetType as any).power?.redundancy || 'none',
        ...
        } else if ((cabinetType as any).cooling?.hasHotAisle && (cabinetType as any).cooling?.hasColdAisle) {
