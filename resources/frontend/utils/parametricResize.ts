/**
 * Параметрический ресайз 3D моделей
 * 
...
export function getResizeRules(obj: THREE.Object3D): ResizeRules {
  ...
  if (shorthand && ['scale', 'move', 'fixed', 'absolute'].includes(shorthand)) {
  ...
  if (rx && ['scale', 'move', 'fixed', 'absolute'].includes(rx)) rules.resize_x = rx as ResizeAction;
  if (ry && ['scale', 'move', 'fixed', 'absolute'].includes(ry)) rules.resize_y = ry as ResizeAction;
  if (rz && ['scale', 'move', 'fixed', 'absolute'].includes(rz)) rules.resize_z = rz as ResizeAction;
...
export function applyParametricResize(
  ...
  // 2. Компенсируем смещение — фиксируем левый-нижний-передний угол
  // После scale от origin (0,0,0) модель сдвигается. Возвращаем на место.
  const offsetX = modelData.min.x * (1 - scaleX);
...
/**
 * Вычисляет новую локальную позицию для оси с move
 * 
...
function calculateNewPosition(
  ...
  // Offset от соответствующего края в оригинале
  // Для anchor='right'/'top'/'back': offset = origSize - origPos
  // Для anchor='left'/'bottom'/'front': offset = origPos
  // Для anchor='center': offset = origPos - origSize/2
  ...
  switch (anchor) {
    ...
    case 'bottom':
    case 'front':
      // Привязка к min краю: сохраняем расстояние от min
      offset = origPos;
      newPos = offset;
      break;
