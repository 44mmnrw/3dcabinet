...
### Workflow: Локальная разработка → GitHub → Production

**Схема работы**:
```
Локальная машина (dev) 
    ↓ git push
GitHub Repository (main/dev)
    ↓ git pull
Production Server (nrwgroup.ru)
```

---

### На локальной машине (после разработки)
...
git add resources/views/landing/index.blade.php

# Создать коммит с описанием
git commit -m "Добавлена новая функция в конфигуратор"
```

#### 3. Отправьте на GitHub
```powershell
# Отправить в ветку dev
git push origin dev

# Или в main (если работаете напрямую в main)
git push origin main
```

---

### На production-сервере (обновление)

#### 1. Подключитесь по SSH
```bash
ssh nrwgroup@your-server-ip
```

#### 2. Получите изменения из GitHub
```bash
cd ~/www/nrwgroup.ru
...
# Или если используете ветку dev:
git pull origin dev
```

#### 3. После обновления кода выполните
```bash
cd ~/www/nrwgroup.ru

# Обновить PHP-зависимости (если изменился composer.json)
php composer.phar install --no-dev --optimize-autoloader

# Применить новые миграции БД (если были изменения)
php artisan migrate --force

# Очистить все виды кеша
php artisan config:clear
php artisan cache:clear
php artisan route:clear
php artisan view:clear

# Пересоздать кеш для production
php artisan config:cache
php artisan route:cache
php artisan view:cache
```
...
### Автоматизация деплоя (deploy.sh)
...
```bash
# Посмотреть что изменено локально
git status

# Вариант A: Сохранить локальные изменения во временное хранилище
git stash
git pull origin main
git stash pop  # Применить обратно

# Вариант B: Отменить ВСЕ локальные изменения (осторожно!)
git reset --hard HEAD
git pull origin main
```

#### "Already up to date" (но изменения не применились)

Проверьте, что изменения отправлены с локальной машины:
...
| Действие | Локальная машина | Production-сервер |
|----------|------------------|-------------------|
| **Посмотреть изменения** | `git status` | `git status` |
| **Отправить изменения** | `git push origin main` | — |
| **Получить изменения** | — | `git pull origin main` |
| **После обновления кода** | — | `php artisan config:clear && php artisan cache:clear` |
