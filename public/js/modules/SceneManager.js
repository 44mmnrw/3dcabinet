...
export class SceneManager {
    constructor(containerElement) {
        ...
        // Параметры комнаты (МЕТРЫ для соответствия GLTF-моделям)
        this.roomWidth = 5.0;   // 5 метров
        this.roomHeight = 3.0;  // 3 метра
        this.roomDepth = 4.0;   // 4 метра
    ...
    init() {
        ...
        // NEAR = 0.01м (10мм), FAR = 1000м (1км) — для метровой сцены
        ...
        // ====================================================================
        // Исходная позиция: (3, 2.5, 3) метров - вид из угла под 45°
        // Для поворота на 45° ВПРАВО от исходного положения:
        //   Исходный азимут: 45° (π/4)
        //   Новый азимут: 0° (вид строго с СЕВЕРА, вдоль оси Z)
        
        const radius = Math.sqrt(3 * 3 + 2.5 * 2.5 + 3 * 3); // Расстояние от центра (в метрах)
        const polarAngle = Math.acos(2.5 / radius);      // Наклон камеры (вертикальный угол)
        ...
        this.controls.minDistance = 0.1;     // Минимум 0.1 м (100 мм) — можно рассмотреть объект
        this.controls.maxDistance = 1000;    // Дальняя дистанция — 1 км
    ...
    createRoom() {
        // Пол (сетка) - divisions = 50 ячеек (по 0.1 м = 10 см каждая для 5-метровой комнаты)
    ...
    setupLighting() {
        ...
        console.log('✅ AmbientLight добавлен (intensity=0.4)');
        
        // ═══════════════════════════════════════════════════════════════
        // 💡 НАПРАВЛЕННОЕ ОСВЕЩЕНИЕ — 3 источника на равном расстоянии
        // ═══════════════════════════════════════════════════════════════
        
        const distance = 4.0;   // Расстояние от центра (4 метра)
        const height = 3.0;     // Высота всех источников (3 метра)
        ...
        light1.shadow.camera.left = -3.0;    // -3 метра
        light1.shadow.camera.right = 3.0;    // +3 метра
        light1.shadow.camera.top = 3.0;      // +3 метра
        light1.shadow.camera.bottom = -3.0;  // -3 метра
        light1.shadow.camera.near = 0.1;     // 0.1 метра
        light1.shadow.camera.far = 8.0;      // 8 метров
        ...
        console.log('  Высота:', height, 'метров');
        ...
        // ═══════════════════════════════════════════════════════════════
        // 🔦 INTERIOR POINT LIGHT — для режима сборки
        // ═══════════════════════════════════════════════════════════════
        // Создаём точечный источник света внутри шкафа (изначально выключен)
        this.interiorPointLight = new THREE.PointLight(0xffffff, 0, 3.0, 2);
        this.interiorPointLight.position.set(0, 0, 0); // Позиция будет обновлена в setInteriorLight()
        this.interiorPointLight.castShadow = false; // Не создаёт тени (оптимизация)
        this.scene.add(this.interiorPointLight);
        
        console.log('✅ InteriorPointLight создан (intensity=0, выключен)');
    }
    ...
    updateCameraClipping(box) {
        ...
        const desiredFar = Math.max(diagonal * 4, 20); // 20 метров минимум
        if (desiredFar > this.camera.far * 0.9) {
            this.camera.far = Math.min(desiredFar, 5000); // 5 км максимум
            this.camera.updateProjectionMatrix();
        }
        const desiredMaxDistance = Math.max(diagonal * 2, 50); // 50 метров минимум
    ...
    resetCamera() {
        ...
        const defaultDistance = 5.0;          // Расстояние от центра (5 метров)
        const defaultPolar = Math.acos(2.5 / Math.sqrt(3 * 3 + 2.5 * 2.5 + 3 * 3)); // Исходный наклон
    ...
    setInteriorLight(enable, cabinetModel = null) {
        ...
        if (enable) {
            ...
            const targetIntensity = 0.5; // Яркость света внутрь (PointLight эффективнее)
